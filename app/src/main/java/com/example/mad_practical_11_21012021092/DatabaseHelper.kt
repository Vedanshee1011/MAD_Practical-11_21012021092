package com.example.mad_practical_11_21012021092
...
class DatabaseHelper(context: Context): SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION){
    ...
    fun getAllPersons() :ArrayList<Person>
    {
        ...
        {
            val id : String = cursor.getString(cursor.getColumnIndex(COLUMN_ID))
            val name : String = cursor.getString(cursor.getColumnIndex(COLUMN_PERSON_NAME))
            val email : String = cursor.getString(cursor.getColumnIndex(COLUMN_PERSON_EMAIL_ID))
            val phone : String = cursor.getString(cursor.getColumnIndex(COLUMN_PERSON_PHONE_NO))
            val address: String = cursor.getString(cursor.getColumnIndex(COLUMN_PERSON_ADDRESS))
            val latitude : Double = cursor.getDouble(cursor.getColumnIndex(COLUMN_PERSON_GPS_LAT))
            val longitude: Double = cursor.getDouble(cursor.getColumnIndex(COLUMN_PERSON_GPS_LONG))
